...
{	
	...
	{
		if (GetTypeId() == TYPEID_PLAYER && map->IsBattleground() && !bg->isArena())
		{
			if (ToPlayer()->GetBGTeam() && ToPlayer()->GetBGTeam() == ALLIANCE)
				ToPlayer()->setFaction(1);
			else if (ToPlayer()->GetBGTeam() && ToPlayer()->GetBGTeam() == HORDE)
...
void Unit::ApplyResilience(Unit const* victim, float* crit, int32* damage, bool isCrit, CombatRating type) const
{
    ...
    switch (type)
    {
        case CR_CRIT_TAKEN_MELEE:
            ...
            if (source && damage)
            {
				if (source->getClass() == CLASS_WARRIOR) // Custom Hackfix for Bugged Warrior Damage (Rly ...)
				{
					if (isCrit)
