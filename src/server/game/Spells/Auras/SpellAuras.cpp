...
void Aura::HandleAuraSpecificMods(AuraApplication const* aurApp, Unit* caster, bool apply, bool onReapply)
{
    ...
    if (apply)
    {	   
		if ((m_spellInfo->AttributesCu & SPELL_ATTR0_CU_AURA_CC) && target->IsControlledByPlayer() || m_spellInfo->Id == 2094 && target->IsControlledByPlayer())
...
void UnitAura::Remove(AuraRemoveMode removeMode)
{
    ...
    if (GetSpellInfo()->Id == 8178) // Grounding totem & Spell Reflection
        return;
